...
from utils.processing import MultiProcessVideoSequencer, MultiProcessVideoSequencerPlusCircleCropping, SingleProcessInferenceVideoSequencer
...
if __name__ == "__main__":
    ...
    parser.add_argument("-of", "--output_folder", type=str, default="cholec80_test", help="Output folder, relative to server['database']['location'].")
    ...
    parser.add_argument("-pp", "--pre_process", type=bool, default=False, help="Pre-process frames in a multiprocess fashion.")
    parser.add_argument("-cf", "--circle_file", type=str, default="cholec80_circle_tracking_individual/df_interpolated/log.pkl", help="Circle cropping information file, relative to server['database']['location'].")
    ...
    if args.multiprocessed:
        if args.pre_process:
            circle_df = pd.read_pickle(os.path.join(server["database"]["location"], args.circle_file))
            ...
            vs = MultiProcessVideoSequencer(  # scaled crop. Discard non-existing indices
                prefix=prefix, 
                postfix=".mp4",
                shape=(240, 320),
                buffer_size=20

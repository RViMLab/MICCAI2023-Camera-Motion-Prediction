...
class WorstSamplingCallback(pl.Callback):
    def __init__(self, worst: float = 0.1, random: float = 0.4) -> None:
        if worst + random > 1.0:
            raise ValueError(
                f"Invalid values, worst + random must be less or equal 1, got {worst} + {random} = {worst + random}."
            )
    ...
    def on_train_batch_end(
        ...
        if covered_batches < total_batches:
            ...
            self._per_sequence_loss_buffer = pd.concat(
                [per_sequence_loss_buffer, self._per_sequence_loss_buffer], axis=0
            )
        else:
            self._per_sequence_loss_buffer.loc[sample_idcs, "loss"] = outputs[
                "per_sequence_loss"
            ].tolist()
    ...
    def on_train_epoch_end(
        ...
        sorted_buffer = self._per_sequence_loss_buffer.sort_values(
            "loss", ascending=False
        )
        sample_idcs = sorted_buffer.iloc[:worst].index
        sample_idcs = sample_idcs.append(
            sorted_buffer.iloc[worst:].sample(random).index
        )

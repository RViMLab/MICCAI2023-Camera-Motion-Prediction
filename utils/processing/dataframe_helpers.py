...
def dataframe_duv_running_average(df: pd.DataFrame, window: int = 20) -> pd.DataFrame:
    ...
    Args:
        df (pd.DataFrame): Dataframe of the form {"folder": str, "file": str, "vid": int, "frame": int, "duv": np.ndarray (4x2)}
    ...
    df_running_averge["duv"] = df_running_averge["duv"].apply(lambda x: x.flatten())

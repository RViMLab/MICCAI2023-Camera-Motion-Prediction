...
class MultiProcessVideoSequencer(object):
    ...
    def start(self, output_prefix: str, processes: int = 2) -> None:
        ...
        log_df_list = p.map(self._sequencing_thread, kwargs_list)
...
class MultiProcessVideoSequencerPlusCircleCropping(MultiProcessVideoSequencer):
    def __init__(
        ...
        shape: Tuple[int, int] = (240, 320),
        buffer_size: int = 10,
        radius_col: str = "radius_running_average_50",
        center_col: str = "center_running_average_50",
    ) -> None:
    ...
    def process_buffer(self, buffer: List[Dict]):
        ...
        for idx, element in enumerate(buffer):
            ...
            if vid_idx != element["vid_idx"]:
                raise RuntimeError(
                    "Wrong video index encountered. Expected {}, found {}.".format(
                        vid_idx, element["vid_idx"]
                    )
                )
            ...
            center.append(row[self._center_col].values[0])
            radius.append(row[self._radius_col].values[0])
            shape.append(row["shape"].values[0])
            img.append(
                cv2.resize(
                    element["img"],
                    (shape[-1][1], shape[-1][0]),
        ...
        if len(buffer) > 0:
            ...
            center, radius, shape = (
                torch.tensor(center),
                torch.tensor(radius),
                torch.Size([len(shape), shape[0][2], shape[0][0], shape[0][1]]),
            )
            box = max_rectangle_in_circle(shape, center, radius)

            # apply transforms
            img = torch.from_numpy(np.stack(img)).permute(
                0, 3, 1, 2
            )  # BxHxWxC -> BxCxHxW
...
class SingleProcessInferenceVideoSequencer:
    ...
    def process_batch(self, batch: List[Dict], log_df: pd.DataFrame) -> pd.DataFrame:
        for sequence in batch:
            ...
            for idx, element in enumerate(sequence):
                file = "frame_{}.npy".format(element["frame_cnt"])
                np.save(
                    os.path.join(element["path"], file),
                    tensor_to_image(imgs[idx], False),
                )
    ...
    def start(self, output_prefix: str) -> None:
        ...
        for idx, row in tqdm(self._df.iterrows()):
            ...
            video_path = os.path.join(self._prefix, row.folder, row.file)
            log_df = pd.DataFrame(
                columns=["folder", "file", "vid", "frame", "center", "radius"]
            )
...
if __name__ == "__main__":
    ...
    vs = MultiProcessVideoSequencer(prefix=prefix, postfix=".mp4", shape=(240, 320))

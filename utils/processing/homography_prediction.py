...
class TaylorHomographyPrediction(HomographyPrediction):
    ...
    def predict(self, duvs: Union[np.ndarray, torch.Tensor], dt: float=1.0) -> Union[np.ndarray, torch.Tensor]:
        converted = False
        if isinstance(duvs, torch.Tensor):
            converted = True
            duvs = duvs.numpy()
        if len(duvs.shape) != 4:
            raise ValueError(f"Expected 4 dimensional input, got {len(duvs.shape)} dimensions.")
        duvs_pred = duvs[:,self._order:].copy()
        for order in range(1, self._order+1):
            # torch.diff doesn't support higher orders
            diff_duv = np.diff(duvs, n=order, axis=1)[:,self._order-order:] # BxNx4x2 -> Bx(N-order)x4x2

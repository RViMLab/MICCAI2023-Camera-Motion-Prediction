...
def integrate_duv(uv: torch.Tensor, duv: torch.Tensor) -> torch.Tensor:
    ...
    if len(duv.size()) != 3:
        raise ValueError(
            "Expected 3 dimensional input for duv, got {} dimensional.".format(
                len(duv.size())
            )
        )
...
def forward_backward_sequence(
    video: Union[np.ndarray, torch.Tensor], step: int = 2, last_step: int = 1

...
def warp_figure(
    img: np.array, uv: np.array, duv: np.array, duv_pred: np.array, H: np.array
) -> plt.Figure:
    r"""
...
def duv_mean_pairwise_distance_figure(
    duvs: np.ndarray, duvs_pred: np.ndarray, re_fps: int, fps: int, dpi: int = 200
) -> plt.Figure:
    ...
    plt.title(
        "Camera Motion: Resampled frame rate {} at {} fps".format(re_fps, int(fps))
    )
    plt.plot(
        np.arange(start=0, stop=duvs_mpd.size) / re_fps, duvs_mpd, label="Estimation"
    )
    plt.plot(
        np.arange(start=duvs_mpd.size - duvs_pred_mpd.size, stop=duvs_mpd.size)
        / re_fps,
        duvs_pred_mpd,
        label="Prediction",
    )
...
def uv_trajectory_figure(
    uv: np.ndarray, uv_pred: np.ndarray, cmap_name: str = "cool", dpi: int = 200
) -> plt.Figure:
    ...
    if len(uv.shape) != 3:
        raise ValueError(
            "Expected 3 dimensional input for uv, got {} dimensional.".format(
                len(uv.shape)
            )
        )
    if len(uv_pred.shape) != 3:
        raise ValueError(
            "Expected 3 dimensional input for uv_pred, got {} dimensional.".format(
                len(uv_pred.shape)
            )
        )
    ...
    lines = [
        Line2D([0], [0], color=c, linestyle="--", marker="o", fillstyle=style)
        for c, style in colors
    ]

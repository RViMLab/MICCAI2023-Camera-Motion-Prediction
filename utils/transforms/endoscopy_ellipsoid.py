...
class EndoscopyEllipsoid(object):
    def movingCenterPipeline(self, 
        ...
        """
        center = EndoscopyEllipsoid.randomCenter(shape=img.shape, scale=c_off_scale, seed=seed)
        half_axes = EndoscopyEllipsoid.randomHalfAxes(shape=img.shape, min_scale=min_scale, max_scale=max_scale, seed=seed)  # assure same half axes
        ...
        center, rot = EndoscopyEllipsoid.randomMotion(img.shape, center=center, rot=rot, scale=dc_scale, dori=dori, chance=update_chance, seed=seed)
        wrp = self(wrp, center=center, half_axes=half_axes, rot=rot)

        return img, wrp

    @staticmethod
    def randomMotion(shape: Tuple[int], center: Tuple[int], rot: float, scale: List[float]=[0.1, 0.1], dori: List[float]=[-np.pi*0.1, np.pi*0.1], chance: float=1., seed: np.int32=None) -> Tuple[np.array, float]:
        ...
        if np.random.rand(1) <= chance:
            new_center = np.random.uniform(
                center-np.array(shape)[:2]*scale, center+np.array(shape)[:2]*scale, 2
            )
    ...
    @staticmethod
    def randomCenter(shape: Tuple[int], scale: List[float]=[0.125, 0.125], seed: np.int32=None) -> np.array:
        ...
        center = np.random.uniform(shape[:2]/2-1 - shape[:2]*scale, shape[:2]/2-1 + shape[:2]*scale)
        np.random.seed(None)
    ...
    @staticmethod
    def randomRot(seed: np.int32=None, low: float=0, high: float=2*np.pi) -> float:
    ...
    def __call__(self, img: np.array, center: np.array=None, half_axes: np.array=None, rot: float=None):
    ...
    def _ellipticNoisyMask(self, img: np.array, center: np.array=None, half_axes: np.array=None, rot: float=None) -> np.array:
        r"""Puts a elliptic noisy mask on top of an image.
        ...
        """
        rr, cc = ellipse(r=center[0], c=center[1], r_radius=half_axes[0], c_radius=half_axes[1], rotation=rot, shape=img.shape)
...
if __name__ == '__main__':
    ...
    if pipe:
        ...
        for _ in range(300):
            ...
            # random update
            center, rot = EndoscopyEllipsoid.randomMotion(img.shape, center=center, rot=rot)

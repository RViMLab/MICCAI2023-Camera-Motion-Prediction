...
class ImageDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None) -> None:
        if stage == "fit" or stage is None:
            self._train_ds = ImageDataset(
                self._train_df, self._prefix, self._train_tranforms, seeds=False
            )
            self._val_ds = ImageDataset(
                self._val_df, self._prefix, self._val_tranforms, seeds=True
            )
        if stage == "test":
            self._test_ds = ImageDataset(
                self._test_df, self._prefix, self._test_tranforms, seeds=True
            )

...
class VideoDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self._train_meta_df, self._val_meta_df = train_test_split(
            self._train_meta_df, train_size=self._train_split, random_state=random_state
        )

        self._train_video_paths = [
            os.path.join(self._prefix, row.database, row.file["path"], row.file["name"])
            for _, row in self._train_meta_df.iterrows()
        ]
        self._val_video_paths = [
            os.path.join(self._prefix, row.database, row.file["path"], row.file["name"])
            for _, row in self._val_meta_df.iterrows()
        ]
        self._test_video_paths = [
            os.path.join(self._prefix, row.database, row.file["path"], row.file["name"])
            for _, row in self._test_meta_df.iterrows()
        ]

        # transforms for each video individually
        self._train_pre_transforms = [
            any_dict_list_to_compose(row.pre_transforms)
            for _, row in self._train_meta_df.iterrows()
        ]
        self._val_pre_transforms = [
            any_dict_list_to_compose(row.pre_transforms)
            for _, row in self._val_meta_df.iterrows()
        ]
        self._test_pre_transforms = [
            any_dict_list_to_compose(row.pre_transforms)
            for _, row in self._test_meta_df.iterrows()
        ]

        self._train_aug_transforms = [
            any_dict_list_to_compose(row.aug_transforms)
            for _, row in self._train_meta_df.iterrows()
        ]
        self._val_aug_transforms = [
            any_dict_list_to_compose(row.aug_transforms)
            for _, row in self._val_meta_df.iterrows()
        ]
        self._test_aug_transforms = [
            any_dict_list_to_compose(row.aug_transforms)
            for _, row in self._test_meta_df.iterrows()
        ]
...
if __name__ == "__main__":
    ...
    for idx, batch in enumerate(dl):
        ...
        print(
            "\rBatch {}/{}, img shape: {}, aug shape: {}, frame_rate: {}, video_fps: {}, Loading time: {}".format(
                idx + 1,
                len(dl),
                img.shape,
                aug.shape,
                fr[0].item(),
                vid_fps[vid_idx[0]][0].item(),

...
class ImageSegmentationDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self._train_df, self._val_df = (
            self._train_df.reset_index(),
            self._val_df.reset_index(),
        )

    def setup(self, stage: str = None):
        if stage == "fit" or stage is None:
            self._train_set = ImageSegmentationDataset(
                self._train_df,
                self._prefix,
                image_transforms=self._train_image_transforms,
                spatial_transforms=self._train_spatial_transforms,
            )
            seeds = np.arange(
                0, len(self._val_df)
            ).tolist()  # assure validation set is seeded the same for all epochs
            self._val_set = ImageSegmentationDataset(
                self._val_df,
                self._prefix,
                image_transforms=self._val_image_transforms,
                spatial_transforms=self._val_spatial_transforms,
                seeds=seeds,
            )
        if stage == "test" or stage is None:
            ...
            self._test_set = ImageSegmentationDataset(
                self._test_df,
                self._prefix,
                image_transforms=self._test_image_transforms,
                spatial_transforms=self._test_spatial_transforms,
                seeds=seeds,
            )
...
if __name__ == "__main__":
    ...
    for batch in dl:
        ...
        img, seg = tensor_to_image(img, keepdim=False), tensor_to_image(
            seg, keepdim=False
        )
        cv2.imshow("img", img)
        cv2.imshow("seg", seg)

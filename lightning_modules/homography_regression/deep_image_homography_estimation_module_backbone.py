...
class DeepImageHomographyEstimationModuleBackbone(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        distance_loss = self._distance_loss(
            duv_fw_pred.view(-1, 2), batch["duv"].to(duv_fw_pred.dtype).view(-1, 2)
        ).mean()
        distance_loss += self._distance_loss(
            duv_bw_pred.view(-1, 2), -batch["duv"].to(duv_bw_pred.dtype).view(-1, 2)
        ).mean()
        consistency_loss = self._distance_loss(
            duv_fw_pred.view(-1, 2), -duv_bw_pred.view(-1, 2)
        ).mean()

        accumulated_loss = distance_loss + consistency_loss

        self.log(
            "train/distance_loss", distance_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(
            "val/distance_loss", distance_loss
        )  # logs all log_every_n_steps https://pytorch-lightning.readthedocs.io/en/latest/logging.html#control-logging-frequency

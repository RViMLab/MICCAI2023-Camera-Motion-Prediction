...
from utils.viz import create_blend_from_four_point_homography, uv_trajectory_figure
...
class PredictiveHorizonModule(pl.LightningModule):
    def __init__(
        ...
        recall_horizon: int = 4,
        preview_horizon: int = 4,
        frame_stride: int = 1,
    ):
        super().__init__()
        ...
        # load model
        self._model = getattr(
            globals()[backbone["module"]], backbone["function"]["name"]
        )(**backbone["function"]["kwargs"], model_num_class=8 * preview_horizon)

...
class NextViewModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            duvs_reg = duvs_reg.view(
                videos.shape[0], -1, 4, 2
            )  # reshape B*Nx4x2 -> BxNx4x2

        # homography prediction
        transformed_frames_i = transformed_videos[
            :, : -self._frame_stride : self._frame_stride
        ]
        transformed_frames_i = transformed_frames_i.reshape(
            (-1,) + frames_i.shape[-3:]
        )  # reshape BxNxCxHxW -> B*NxHxW

        duvs = self(
            transformed_frames_i
        )  # forward transformed correspondence to frames_i
        duvs = duvs.view(videos.shape[0], -1, 4, 2)  # reshape B*Nx4x2 -> BxNx4x2

        # distance loss
        distance_loss = self._distance_loss(
            duvs.view(-1, 2), duvs_reg.view(-1, 2)
        ).mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not self._val_logged:
            ...
            duv_mpd_seq_figure = duv_mean_pairwise_distance_figure(
                duvs_reg[0].cpu().numpy(),
                re_fps=frame_rate[0].item(),
                fps=vid_fps[vid_idc[0]][0].item(),
            )  # get vid_idc of zeroth batch
            self.logger.experiment.add_figure(
                "val/duv_mean_pairwise_distance", duv_mpd_seq_figure, self.global_step
            )

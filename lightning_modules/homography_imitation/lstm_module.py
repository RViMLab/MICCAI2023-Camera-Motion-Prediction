...
class FeatureLSTMModule(pl.LightningModule):
    ...
    def forward(self, imgs_ip1: torch.Tensor, duvs_i: torch.Tensor, dduvs_im1: torch.Tensor) -> torch.Tensor:
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        return {
            "loss": loss.mean(),
            'per_sequence_loss': loss.detach().view(duvs_ip1.shape[0], -1).mean(axis=-1).cpu().numpy()
        }

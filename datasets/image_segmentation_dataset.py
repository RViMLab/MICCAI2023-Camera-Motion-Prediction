...
class ImageSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        if seeds:
            if len(df) != len(seeds):
                raise Exception(
                    "In ImageSegmentationDataset: Length of dataframe must equal length of seeds."
                )
    ...
    def __getitem__(self, idx):
        ...
        img = np.load(
            os.path.join(
                self._prefix,
                self._df.iloc[idx].image["folder"],
                self._df.iloc[idx].image["file"],
            )
        )
        seg = np.load(
            os.path.join(
                self._prefix,
                self._df.iloc[idx].segmentation["folder"],
                self._df.iloc[idx].segmentation["file"],
            )
        )
